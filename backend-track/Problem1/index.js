...
const vehiclesTasksData = [
  ...
  { "TaskID": "a1e0b8e6-1076-42a2f-b83b-5e6017900033", "Duration": 6, "Impact": 1 },
...
app.get('/schedule-tasks/:depotId', (req, res) => {
  const depotId = parseInt(req.params.depotId);
  const depot = depotsData.find(d => d.ID === depotId);

  if (!depot) {
    return res.status(404).json({ error: "Depot ID not found" });
  }

  const capacity = depot.MechanicHours;
  const n = vehiclesTasksData.length;

  const dp = Array(n + 1).fill().map(() => Array(capacity + 1).fill(0));

  for (let i = 1; i <= n; i++) {
    for (let w = 0; w <= capacity; w++) {
      if (vehiclesTasksData[i - 1].Duration <= w) {
        dp[i][w] = Math.max(
          vehiclesTasksData[i - 1].Impact + dp[i - 1][w - vehiclesTasksData[i - 1].Duration],
          dp[i - 1][w]
        );
      } else {
        dp[i][w] = dp[i - 1][w];
  ...
  for (let i = n; i > 0 && w > 0; i--) {
    if (dp[i][w] !== dp[i - 1][w]) {
      selectedTasks.push(vehiclesTasksData[i - 1]);
      w -= vehiclesTasksData[i - 1].Duration;
    }
  }

  const totalDurationSpent = selectedTasks.reduce((sum, item) => sum + item.Duration, 0);
  const totalMaximizedImpact = dp[n][capacity];

  res.json({
    depotID: depotId,
    availableMechanicHours: capacity,
    totalDurationUtilized: totalDurationSpent,
    totalMaximizedOperationalImpact: totalMaximizedImpact,
    scheduledTasks: selectedTasks.reverse()
  });
});

app.listen(PORT, () => {
  console.log(`Vehicle Maintenance Scheduler Microservice running on port ${PORT}`);
});